...
import eg.edu.alexu.csd.oop.game.DesignPattern.PlayingState;
...
public class MyWorld implements World, Observable, Serializable {
	...
	public void initialize() {
		...
		// control objects (men)
		pool = new ShapePool(men, "Man", width / 3, (int) (height * 0.8) - 60);
	...
	public void doStateAction(){
		 if(gs.getState().toString().equals("Pause State")){
			...
			if(action != null && action.equals("Save Game")){
				...
				gs.setState(new PlayingState(gs.getState().getController()));
	...
	private StackContainer getStackOfMan(StackCarrier man, boolean isRight){
		...
		if(isRight) return (StackContainer) control.get(men+ ((i * 2) + 1));
		return (StackContainer) control.get(men+ ((i * 2)));
	}
